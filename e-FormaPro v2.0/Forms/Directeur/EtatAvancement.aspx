     ...
     <table align="center">
        ...
        <tr>
            <td  colspan = "3" align="center">
                ...
                <asp:SqlDataSource ID="SqlDataSource_Formateur" runat="server" ConnectionString="<%$ ConnectionStrings:Espace_pédagogique2FatiConnectionString %>" SelectCommand="SELECT *, (Formateurs.Nom + ' ' + Formateurs.Prenom) [Nom complet] FROM [Formateurs]" ProviderName="<%$ ConnectionStrings:Espace_pédagogique2FatiConnectionString.ProviderName %>"></asp:SqlDataSource>
                <br />
                ...
                <asp:SqlDataSource ID="SqlDataSource_Avancement" runat="server" ConnectionString="<%$ ConnectionStrings:Espace_pédagogique2FatiConnectionString %>" SelectCommand="SELECT SUM(Seances.Duree) AS Cumule, Modules.MassHorraire - SUM(Seances.Duree) AS Rest FROM Seances INNER JOIN Modules ON Modules.Id_Module = Seances.Module INNER JOIN Formateurs ON Formateurs.Matricule = Seances.Formateur INNER JOIN Groupes ON Groupes.Id_Groupes = Seances.Groupe GROUP BY Formateurs.Matricule, Groupes.Id_Groupes, Modules.Id_Module, Modules.MassHorraire HAVING (Formateurs.Matricule = @formateur) AND (Groupes.Id_Groupes = @groupe) AND (Modules.Id_Module = @module)" ProviderName="<%$ ConnectionStrings:Espace_pédagogique2FatiConnectionString.ProviderName %>">
                    <SelectParameters>
